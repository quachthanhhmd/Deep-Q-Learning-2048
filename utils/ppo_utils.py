...
class PPORolloutBuffer:
    ...
    def __init__(self, obs_dim, num_actions, num_steps, num_games, device="cpu"):
        ...
        self.steps = torch.zeros((num_steps, num_games), device=device)
    ...
    def add(self, rollout_step, obs, action, logprob, reward, done, value, valid_mask, step_in_game=1.0):
        ...
        self.steps[rollout_step] = step_in_game
    ...
    def get_batches(self, next_value, next_done, gamma, gae_lambda):
        ...
        b_steps = self.steps.reshape(-1)
        b_masks = self.valid_masks.reshape((-1,) + self.valid_masks.shape[2:])
        
        return b_obs, b_logprobs, b_actions, b_advantages, b_returns, b_values, b_masks, b_steps
